...
def _exec(name: str, script: str) -> "tuple[int, str]":
...
def cmd_install(args):
    ...
    user_name = meta.get_meta(args.name).get("user", "petri")
    ...
    if getattr(args, "agent", None):
        ...
        if agent.get("packages"):
            rc, out = _exec(args.name, "dnf install -y " + " ".join(agent["packages"]))
            if rc != 0:
        ...
        for pkg in agent.get("mise_packages", []):
            ...
            rc, out = _exec(args.name, f"export HOME={home} && {home}/.local/bin/mise use -g {pkg}")
            if rc != 0:
        ...
        if agent.get("install_script"):
            ...
            rc, out = _exec(args.name, f"export HOME={home} && {agent['install_script']}")
            if rc != 0:
